...
def qualify():
    ...
    client = sqlite3.connect("../Task4.db")
    curr = client.execute("SELECT competitor.name, SUM(scores.score), SUM(scores.score) > 250 FROM competitor INNER JOIN scores ON competitor.id = scores.id GROUP BY competitor.id")
    attr = list(curr)

    return render_template("qualify.html", attr = attr)    
